...
import { ApprovePayoutButton, PayPayoutButton } from "../components/payout-actions.js";
import { getPayouts, requireCurrentMember, type PayoutSummary } from "../queries.js";
...
function formatReleaseDate(releaseAt: string) {
  return new Date(releaseAt).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" });
}

const statusVariantMap: Record<PayoutSummary["status"], "default" | "secondary" | "outline" | "destructive"> = {
  held: "outline", eligible: "secondary", approved: "secondary", paid: "default", voided: "destructive",
};
...
/** Camp's Payouts view: the first migrated screen that also exercises a
 * real mutation path (approve/pay), not just reads -- proving the config
 * singleton works for server actions, not only server components. See
 * TandemPayoutAdapter and its Stripe Connect reference implementation for
...
export async function PayoutsView() {
  ...
  const pendingPayouts = payouts.filter((p) => p.status === "held" || p.status === "eligible");
  const totalsByCurrency = new Map<string, number>();
  for (const payout of pendingPayouts) {
    totalsByCurrency.set(payout.currency, (totalsByCurrency.get(payout.currency) ?? 0) + payout.amountMinor);
  }
  const pendingTotalSummary = Array.from(totalsByCurrency.entries()).map(([c, a]) => formatCurrency(a, c)).join(" + ");
  const pendingDescription = pendingPayouts.length === 0
    ? "No pending payouts"
