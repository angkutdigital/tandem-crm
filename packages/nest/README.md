...
sales activity (Trail), commissions, and disputes (Coaster) — into your own
...
into it. See the repo root's `HANDOFF.md` (2026-09-26, "product direction
...
was never going to be the weight problem — Core alone compiles to 16 KB.
...
must never see any of `tandem-nest`'s dependencies in their lockfile at
...
screens into this package behind a real mount API (`mountTandemAdmin()` or
...
host not using Coaster) doesn't ship its JS to every page.
